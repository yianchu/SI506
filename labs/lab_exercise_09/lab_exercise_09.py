...
class Person:
    ...
    def jsonable(self):
        ...
        """
        homeworld = None
        if self.homeworld: #if self.homeworld != None
...
def main():
    ...
    # step 2
    luke = Person(swapi_luke['url'], swapi_luke['name'], swapi_luke['birth_year'], float(swapi_luke['height']), float(swapi_luke['mass']))

    #print(f"Luke Skywalker's Profile = {luke}\n")

    # step 3
    tatooine_data = read_json('tatooine.json')
    #print(f"tatooine_data = {tatooine_data}")

    # step 4
    swapi_tatooine = get_swapi_resource(ENDPOINT + '/planets/', {'search':'tatooine'})['results'][0]
    #print(f"swapi_tatooine's Data From SWAPI = {swapi_tatooine}\n")

    # step 5
    swapi_tatooine.update(tatooine_data)

    # step 6
    tatooine = Planet(swapi_tatooine['name'], float(swapi_tatooine['diameter']), swapi_tatooine['climate'], swapi_tatooine['system'], int(swapi_tatooine['population']), swapi_tatooine['orbital_position'])
