...
def main():
    ...
    lars_data = get_swapi_resource(ENDPOINT + '/people/', {'search': 'owen lars'})['results'][0]
    ...
    lars_species = get_swapi_resource(lars_data['species'][0])['name']


    lars_data['lars_species'] = lars_species
    #print(f"Owen Lars data w/Species = {lars_data}")

    # Update values in < lars >.
    for i in lars_data.keys():
        if i in lars.keys():
            lars[i] = lars_data[i]
